...
# a = list(map(int, a)) # Чтобы превратить список в числа.
# Если лист не isdigit, то тогда через list Comprehension.
# ------------------------------------------------------------------------------------------------
...
# operator_1 = {'+': lambda x, y: x + y, # Создал словарь 1.
#             '-': lambda x, y: x - y}
# operator_2 = {'*': lambda x, y: x * y, # Словарь 2.
#             '/': lambda x, y: x / y} 
...
#         # operator_2[my_string[index]] - таким образом я спрашиваю, есть ли умножение или деление в 
#         # списке, если он есть тогда запускается нужная мне lambda функция. (my_string[index - 1], 
#         # my_string[index + 1]) - указываю координаты аргументов, для функции lambda.
#         del my_string[index - 1: index + 2] # del - удаляет элементы из списка 
#         # my_string[от index-1 до index+2], т.е. удалил (2 * 3)
#         my_string.insert(index - 1, temp) # .insert(позиция, элемент) - позволяет вставлять элемент в список.
#         index = 0 # Сбрасываю счетчик, повторяю цикл снова.
#     else:
...
# --------------------------------------------------------------------------------------------------
# РЕШЕНИЕ 1 через list Comprehension ----------------------------------------------------------------
# incoming_list = [1, 2, 3, 5, 1, 5, 3, 10]
# outcoming_list = [i for i in incoming_list if incoming_list.count(i) == 1] 
# # Иду по (incoming_list) и если (i) элемент встречается только один раз (incoming_list.count(1) == 1, 
# # то добавляю его в (outcoming_list)
...
# ---------------------------------------------------------------------------------------------------
# filter --------------------------------------------------------------------------------------------
# res = list(filter(lambda x: x > 2, input_list)) # Пишу list чтобы получить на выходе тип данных список
# print(res) # [3, 5, 5, 3, 10]
# res2 = tuple(filter(lambda x: x > 2, input_list)) # Пишу tuple чтобы получить тип данных КОРТЕЖ.
...
# res4 = list(map(lambda x: int(x) if x.isdigit() else x, input_list2)) # Тут запись через тернарный оператор (if else)
