...
token = environ['token_bot'] # token_bot
token_accu = environ['token_accu'] # token_accu
...
# Для того чтобы получить прогноз от accuweather нужен уникальный код города, чтобы его получить 
# нужны его координаты, так же координаты города нам понадобятся при запросе к яндексу

def code_location(latitude: str, longitude: str, token_accu: str): # Функция получения кода города.
    ...
    code = json_data['Key']
    return code
# На вход функции даем координаты и токен, префикс “f” перед строкой дает нам вставлять значения 
# переменных в нужные места вместо текста. Далее мы отправляем запрос по указанному url и с нужным 
# заголовком. Хоть resp_loc.text и выглядит как стандартный словарь python на самом деле это 
# json-объект, его нужно переделать в словарь, по ключу ‘Key’ получаем код города.

def weather(cod_loc: str, token_accu: str): # Функция получения непосредственно прогноза.
...
# По ключу ‘link’ сохраним ссылку на полный прогноз, сохраним погоду сейчас, далее нам понадобится 
# в цикле перебрать оставшиеся часы и так же сохранить их в словаре

def yandex_weather(latitude, longitude, token_yandex: str): # Функция получения погоды от яндекса
    ...
    return pogoda
# нюанс направление ветра и состояние неба в ответе от яндекса пишется на английском языке, 
# делаем словари conditions и wind_dir для перевода. В ответе только фактическая погода и прогноз 
# на следующих два периода (можно сделать и более подробно, но в бесплатном варианте только так).  
# Сразу в словаре меняем направление ветра и состояние неба. Далее создаем новый словарь и забираем 
# только то что нам нужно из всего ответа. При отладке кода стоит отметить, что в бесплатной 
# версии api яндекса допускается только 50 запросов в сутки.

def print_weather(dict_weather, message): # Функции которые будут отправлять ответом прогнозы. Начнем с аккувезер
...
# В эту функцию я должен погрузить вывод функции weather и сообщение на которое я отвечаю. 
# В самой функции я отправляю сообщение с текстом и ссылками на тот словарь из функции weather. 
# В конце отправляю сообщение со ссылкой на полный прогноз.

def print_yandex_weather(dict_weather_yandex, message): # Функция делает сообщение с погодой от яндекса.
...
# скармливаем функции выход функции yandex_weather и сообщение на которое отвечаем. Здесь создаем 
# словарь с переводом времени дня, затем отправляем сообщение, далее задаем цикл по ключам словаря 
# из выхода функции yandex_weather если ключ не link то отправляем сообщение: температура {время дня}, 
# на небе {состояние неба}. В конце отправляем ссылку на яндекс

def big_weather(message, city): # Обобщающая функция
...
# В нее я погружаю сообщение и город. Определяю координаты, запрашиваю accuweather и пишу сообщение 
# на основе данных от него, потом тоже самое от яндекса.

def add_city(message): # Функция сохранения городов.
    try:
        ...
        return cities, 1
# Сперва проверяю существование такого города если все ок то все что после слова город сохраняю 
# в словарь cities под ключом id пользователя. Можно было бы использовать имя, но это только если я
# уверен, что сервисом будут пользоваться люди с разными именами.

@bot.message_handler(command=['start', 'help']) # Функция ответа на сообщения.
...
# В первой части если автор сообщения сказал «привет» или «здарова» мы просто здороваемся и 
# напоминаем, то что мы ждем от автора сообщения.
# Если написал «погода» то уточняемся есть ли он в нашем списке и если есть, то сообщаем 
# название города и выполняем функцию big_weather.
# Если пользователя нет, то предлагаем ему задать его стандартный город.
# Если пользователь написал «Мой город *****», то пользователя. И сообщаем что город записан, 
# если что-то пошло не так, то сообщаем об этом.
# Во всех остальных случаях мы сообщение воспринимаем как название города. 
# Если города такого не находится то сообщаем об этом.
...
# Здесь мы создаем объект bot, потом открываем json файл с архивом городов и id пользователей. 
# И наконец запускаем в бесконечном цикле бота.
